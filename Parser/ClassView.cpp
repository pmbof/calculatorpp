...
void CClassView::FillClassView()
{
	...
	CDocument* pDoc = pParentFrm->GetActiveDocument();

	HTREEITEM hRoot = m_wndClassView.InsertItem(_T("Parser debuger"), 0, 0);
	m_wndClassView.SetItemState(hRoot, TVIS_BOLD, TVIS_BOLD);

	if(pDoc && pDoc->IsKindOf(RUNTIME_CLASS(CParserDoc)))
	{
		HTREEITEM hClass = m_wndClassView.InsertItem(_T("Iterators"), 1, 1, hRoot);
		m_wndClassView.InsertItem(_T("Space"), 3, 3, hClass);
		m_wndClassView.InsertItem(_T("Alpha"), 3, 3, hClass);
		m_wndClassView.InsertItem(_T("Number"), 3, 3, hClass);
		m_wndClassView.InsertItem(_T("Parentheses"), 3, 3, hClass);
	}
	m_wndClassView.Expand(hRoot, TVE_EXPAND);

	HTREEITEM hClass = m_wndClassView.InsertItem(_T("CFakeApp"), 1, 1, hRoot);
